...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy((element: Char) => element).map((e)=>(e._1, e._2.length)).toList.sorted
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val flatWordLists = s.flatMap { x =>  wordOccurrences(x)} 
    
    val m = flatWordLists.groupBy(p=>p._1)
    val m2 = m.map( p => p match {
      case ( ch, lst) => (ch, lst.unzip._2.sum)
    })
       m2.toList.sorted
  }
  ...
  def wordAnagrams(word: Word): List[Word] = {
    val temp = wordOccurrences(word)
    val temp2 = dictionaryByOccurrences get temp
    val temp3 = temp2 match {
      case Some(x) => x;
      case None => List();
    }
    temp3;
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def recGetCombos(aggregComb: List[Occurrences], occurrencesToAdd : List[Occurrences]) : List[Occurrences] = {
          ...
          recGetCombos(newAggregComb, newOccurrencesToAdd.distinct )
             // newOccurrencesToAdd   
         }
    }
    
    recGetCombos(List(),recGetCombos(List(), List(occurrences)))
  }
    /*
      def helperAddReducedElement(curList2: Occurrences, element: (Char, Int)) = {
        if (element._2-1 != 0){ ((element._1, element._2 -1)::curList2).sortBy(_._1) }
        else { curList2}
    }
    ...
    recursiveAddReduced(List(), List(occurrences))
  ...
  */
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def getWordList(sentOccurrence: Occurrences) : List[Word] = {
      val temp2 = dictionaryByOccurrences get sentOccurrence
      val temp3 = temp2 match {
      case Some(x) => x;
      case None => List();
      }
